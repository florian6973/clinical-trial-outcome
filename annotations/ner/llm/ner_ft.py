...
if __name__ == "__main__":
    ...
    train_dataset = load_dataset(
        "../data/manual-ann-ner-100_200.csv",
        "../data/manual-ann-ner-extended.yaml"
    )
    test_dataset = load_dataset(
        "../data/manual-ann-ner-0_100.txt",
        "../data/manual-ann-ner.yaml"
    )
    ...
    folder = "./finetuned_clinicalbiobert"
    num_labels = 7

    if inference:
        folder_inf = "./finetuned_clinicalbiobert_inf"
        import torch
        tokenizer = AutoTokenizer.from_pretrained(folder_inf, device_map='cuda:0')
        model = AutoModelForTokenClassification.from_pretrained(folder_inf, num_labels=num_labels, device_map='cuda:0')
        ...
        # print(outcome2)
        batch_size = 100 # 1000 batch size on gpu possible
        outcomes_json = []
        for i in tqdm(range(0, len(rows), batch_size)):
            outcome2 = [rows[k+i] for k in range(batch_size)]

            outcome2_pp = [preproc(outcome) for outcome in outcome2]
            ...
            # print(len(outcome_words))
                ...
                outcomes_json.append((outcome2[i], inf_st))

                if i % 10000 == 0:
                    with open('outcomes_extracted.json', 'w') as f:
    ...
    # tokens = tokenizer.convert_ids_to_tokens(tokens["input_ids"][0])
        ...
        # tokenizer(outcome)
        # text, return_tensors="pt", truncation=True, is_split_into_words=True
    else:
        evals = {}
        ...
        print(evals)
        import json
        with open('perf_3.json', 'w') as f:
            json.dump(evals, f)


            # print(model(dataset['test'][:2]))